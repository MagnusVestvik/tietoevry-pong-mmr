...
func NewAuthService(employeeRepo *repos.EmployeeRepo) *AuthService {
	return &AuthService{
...
func (as *AuthService) Login(email, pass string) (*string, error) {
	employee, err := as.employeeRepo.GetEmployeeByEmail(email)
	if err != nil {
		return nil, err
	}

	passwordMatchesHash, err := as.verifyPassword(pass, employee.Password)
	if err != nil {
